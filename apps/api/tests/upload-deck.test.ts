...
function buildValidStageZip(): Buffer {
...
describe("POST /api/decks (upload)", () => {
  it("rejects upload without a session", async () => {
    ...
    form.append(
      "file",
      new File([new Uint8Array(zip)], "test.stage", { type: "application/zip" }),
    );
  ...
  it("accepts a valid .stage upload and returns 201", async () => {
    ...
    const body = (await res.json()) as {
      ...
      manifestSummary: { slideCount: number; schema: string };
    };
    ...
    const blob = await ctx.app.request(
      `http://127.0.0.1:3000/api/decks/${body.id}/blob`,
      { headers: { Cookie: cookieHeader(userCookies) } },
    );
  ...
  it("rejects a non-zip payload with INVALID_STAGE_ZIP", async () => {
    ...
    form.append(
      "file",
      new File([new Uint8Array(Buffer.from("not a zip"))], "broken.stage", {
  ...
  it("rejects a zip without manifest.json", async () => {
    const zip = Buffer.from(
      zipSync({ "slides/01.html": strToU8("<html></html>") }),
    );
    const form = new FormData();
    form.append(
      "file",
      new File([new Uint8Array(zip)], "nomf.stage", { type: "application/zip" }),
    );
    const res = await ctx.app.request("http://127.0.0.1:3000/api/decks", {
      method: "POST",
      headers: { Cookie: cookieHeader(userCookies) },
