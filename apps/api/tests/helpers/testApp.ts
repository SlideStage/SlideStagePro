...
import { createApp, type BuiltApp } from "../../src/server.js";
...
let workerEnv: TestEnv | null = null;
...
export interface BuildTestAppOptions {
  ...
  storage?: import("../../src/storage/types.js").StorageDriver;
}
...
export async function signUpUser(
  ...
  if (userCount > 0 && !inviteToken) {
    ...
    if (!creator) throw new Error("no creator available");
  ...
  const sch = (res.headers as Headers & {
    getSetCookie?: () => string[];
  }).getSetCookie?.();
  if (sch && sch.length > 0) {
...
export async function signInUser(
  ...
  if (sch && sch.length > 0) cookies.push(...sch);
  else {
