...
function buildStageZip(title = "Notes Deck") {
    const manifest = {
...
beforeAll(async () => {
    ...
    form.append("file", new File([new Uint8Array(buildStageZip())], "deck.stage", {
...
describe("PUT /api/decks/:id/notes/:slideIndex", () => {
    it("upserts a note for slide 0", async () => {
        const putRes = await ctx.app.request(`http://127.0.0.1:3000/api/decks/${deckId}/notes/0`, {
            method: "PUT",
            headers: {
        ...
        // Overwrite & re-read.
        await ctx.app.request(`http://127.0.0.1:3000/api/decks/${deckId}/notes/0`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                Cookie: cookieHeader(cookies),
        ...
        const listRes = await ctx.app.request(`http://127.0.0.1:3000/api/decks/${deckId}/notes`, { headers: { Cookie: cookieHeader(cookies) } });
    ...
    it("rejects an empty note body that exceeds 10k chars", async () => {
        ...
        const res = await ctx.app.request(`http://127.0.0.1:3000/api/decks/${deckId}/notes/0`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                Cookie: cookieHeader(cookies),
    ...
    it("DELETE removes the note", async () => {
        const del = await ctx.app.request(`http://127.0.0.1:3000/api/decks/${deckId}/notes/0`, { method: "DELETE", headers: { Cookie: cookieHeader(cookies) } });
        expect(del.status).toBe(204);
        const list = await ctx.app.request(`http://127.0.0.1:3000/api/decks/${deckId}/notes`, { headers: { Cookie: cookieHeader(cookies) } });
